...
```{r}
# synth.ETAS <- function(t1 = NULL, t2 = NULL, envir = etas$envir,
#                      ht = etas$envir$data[which.max(
#                          etas$envir$data$magnitudes), ]){
#     
#     # updates environments if specified by users
...
```{r}
# # modelling
#     post.par <- matrix(rep(0, n.cat * 5), ncol = 5)
#     Npostmean <- rep(0, n.cat)
#     Nlarge <- rep(0, n.cat)
#     abs_dist_int <- rep(0, n.cat) 
#     abs_dist_mag <- rep(0, n.cat)
#     overdisp <- rep(0, n.cat)
#     j <- rep(0, n.cat)
# 
#     i <- 1
#     while(i <= n.cat){
#         Nevents <- nrow(multi.synth.cat.df[
#             multi.synth.cat.df$cat.idx == i,])
#         if(Nevents >= events_threshold){
...
#     }
#     
#     # remove the elements not evaluated
#     k <- which(j == 1)
#     post.par <- post.par[-k,]
#     Npostmean <- Npostmean[-k]
#     Nlarge <- Nlarge[-k]
#     abs_dist_int <- abs_dist_int[-k]
#     abs_dist_mag <- abs_dist_mag[-k]
#     overdisp <- overdisp[-k]
```

...
mult.synth.ETAS <- function(t1 = NULL, t2 = NULL, n.cat = 500,
    ...
    Nevents <- unlist(lapply(1 : n.cat, \(i) nrow(
...
synth.fit <- function(breaks = c(0, 125, 150, 200, 400, 1600),
    ...
    multi.synth.cat.plot <- ggplot(cat.df.for.plotting,
      ...
      geom_point(
        ...
        mapping = aes(ts, magnitudes), color = "black"
      ) +
    ...
    Npost <- vector(mode = 'list', length(levels(classes)))
    ...
    post[[1]]$post.df$Catalogues <- 
        'Random Catalogue 1: Less than 125 Events'
    ...
    Npost[[1]]$post.df$Catalogues <- 
        'Random Catalogue 1: Less than 125 Events'
    Npost[[2]]$post.df$Catalogues <- 'Random Catalogue 2: 125 to 150 Events'
    Npost[[3]]$post.df$Catalogues <- 'Random Catalogue 3: 150 to 200 Events'
    Npost[[4]]$post.df$Catalogues <- 'Random Catalogue 4: 200 to 400 Events'
    Npost[[5]]$post.df$Catalogues <- 
        'Random Catalogue 5: 400 to 1600 Events'
    ...
    post.N.plot <- ggplot(bind.post.N.df,
                          aes(x = N, y = mean, color = Catalogues)) +
      geom_line() +
      xlab("N") +
      ylab("pdf") +
      geom_vline(

...
ggplot() +
  ...
  geom_sf(data = italy.map, fill = alpha("lightgrey", 0), colour = 'green',
          linewidth = 0.7) +
  geom_sf(data = aquila.sf[aquila$M > 5,], size = 0.5, colour = 'orange') +
  geom_sf(data = aquila.sf[aquila$M > 6,], size = 0.6, colour = 'red') +
...
mult.synth.ETAS <- function(t1 = NULL, t2 = NULL, n.cat = 500,
    ...
    # calculates the number of events in each catalogue
    Nevents <- unlist(lapply(seq_len(n.cat), \(i) nrow(
        multi.synth.cat.list.df[[i]])))
...
```{r, message = FALSE, warning = FALSE}
synth.fit <- function(breaks = c(0, 125, 150, 200, 400, 1600),
                      samp.each.class = 1){
    
    # selecting catalogues
    Nevents <- mult.synth$environ$Nevents
    classes <- cut(Nevents, breaks = breaks)
    ...
    cat.df.for.plotting <- rbind(
      mult.synth$environ$multi.synth.cat.df[
        which(
          mult.synth$environ$multi.synth.cat.df$cat.idx %in% samp.id), 
        ],
      cbind(mult.synth$environ$envir$data[, c("ts", "magnitudes")],
    ...
    multi.synth.cat.plot <- ggplot(cat.df.for.plotting,
      ...
      geom_point(size = 0.5) +
      geom_point(
        data = mult.synth$environ$ht, 
        mapping = aes(ts, magnitudes), colour = "black"
      ) +
      facet_wrap(facets = vars(cat.idx, num_events), 
                 labeller = 'label_both')
    
    # modelling
    input <- vector(mode = 'list', classes %>% levels %>% length)
    post <- vector(mode = 'list', classes %>% levels %>% length)
    post.par <- matrix(rep(0, (classes %>% levels %>% length) * 5), 
                       ncol = 5)
    Npost <- vector(mode = 'list', classes %>% levels %>% length)
    Npostmean <- rep(0, classes %>% levels %>% length)
    Nlarge <- rep(0, classes %>% levels %>% length)
    abs_dist_int <- rep(0, classes %>% levels %>% length)
    abs_dist_mag <- rep(0, classes %>% levels %>% length)
    overdisp <- rep(0, classes %>% levels %>% length)
    
    for(i in classes %>% levels %>% length %>% seq_len){
        multi.synth.etas <- ETAS(data = 
            mult.synth$environ$multi.synth.cat.list.df[[samp.id[i]]],
    ...
    post[[1]]$post.df$Catalogues <- 
        paste('Random Catalogue 1: Less than', breaks[2], 'Events')
    for(i in 2 : length(samp.id)){
        post[[i]]$post.df$Catalogues <- 
        paste('Random Catalogue', i, ':', breaks[i], 'to', breaks[i + 1],
              'Events')
    }
    ...
    # bind marginal posterior data.frames
    bind.post.N.df <- rbind(Npost[[1]]$post.df, Npost[[2]]$post.df, 
                          Npost[[3]]$post.df, Npost[[4]]$post.df, 
                          Npost[[5]]$post.df)
...
mult.synth.fit <- synth.fit()
```

Analysis on the Behaviours of the Time-between-Events

```{r}
ECDF.interarrival <- function(){
    
    samp.id <- mult.synth.fit$environ$samp.id
    Nevents <- mult.synth$environ$Nevents
    ...
    data.list <- lapply(samp.id %>% length %>% seq_len, \(i) data.frame(
        Time_between_events = 
            mult.synth$environ$multi.synth.cat.list.df[[samp.id[i]]]$ts %>%
                sort %>% diff, 
        Num_Events = paste(Nevents[samp.id[i]], '(synthetic)')
        )
    )
    
    data.list[[length(samp.id) + 1]] <- data.frame(
        Time_between_events = etas$envir$interarrival, 
        Num_Events = paste(nrow(etas$envir$data), '(observed)'))
    
    data.list[[length(samp.id) + 2]] <- data.frame(
        Time_between_events = 
            rexp(length(etas$envir$interarrival), 
                 1 / etas$envir$m_int_time), 
        Num_Events = paste(nrow(etas$envir$data), 
...
ecdf_interarrival <- ECDF.interarrival()
ecdf_interarrival
```
...
```{r}
save.image(file = 'Robin_new.RData')
```

...
mult.synth.imp <- mult.synth.ETAS(ht = etas$envir$data[which.max(
                         etas$envir$data$magnitudes), ])
```
...
synth.fit <- function(samp.each.class = 5, 
    ...
    characteristic <- switch(charac_id, Nevents, Nlar, MaxMag)
    breaks <- switch(charac_id,
                quantile(characteristic, 
                              probs = 
                    seq(0, 17 / 20, length.out = 4)[c(bin_id, bin_id + 1)]),
                quantile(characteristic, 
                              probs = 
                    seq(0, 17 / 20, length.out = 4)[c(bin_id, bin_id + 1)]),
                quantile(characteristic, 
...
mult.synth.fit <- rep(list(NULL), 9)
mult.synth.fit[[1]] <- synth.fit(syn = mult.synth.imp, 
                                 charac_id = 1, bin_id = 1)
mult.synth.fit[[2]] <- synth.fit(syn = mult.synth.imp, 
                                 charac_id = 1, bin_id = 2)
mult.synth.fit[[3]] <- synth.fit(syn = mult.synth.imp, 
                                 charac_id = 1, bin_id = 3)

mult.synth.fit[[4]] <- synth.fit(syn = mult.synth.imp, 
                                 charac_id = 2, bin_id = 1)
mult.synth.fit[[5]] <- synth.fit(syn = mult.synth.imp, 
                                 charac_id = 2, bin_id = 2)
mult.synth.fit[[6]] <- synth.fit(syn = mult.synth.imp, 
                                 charac_id = 2, bin_id = 3)

mult.synth.fit[[7]] <- synth.fit(syn = mult.synth.imp, 
                                 charac_id = 3, bin_id = 1)
mult.synth.fit[[8]] <- synth.fit(syn = mult.synth.imp, 
                                 charac_id = 3, bin_id = 2)
mult.synth.fit[[9]] <- synth.fit(syn = mult.synth.imp, 
                                 charac_id = 3, bin_id = 3)
```
...
ecdf_interarrival <- lapply(seq_len(9), \(j) ECDF.interarrival(j))
```

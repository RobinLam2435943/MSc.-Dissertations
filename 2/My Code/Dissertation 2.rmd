...
```{r, message = FALSE, warning = FALSE}
synth.fit <- function(breaks = c(0, 110, 130, 150, 170, 190, 210),
                      samp.each.class = 1){
    ...
    # modelling
    input <- rep(list(NULL), classes %>% levels %>% length)
    post <- rep(list(NULL), classes %>% levels %>% length)
    post.par <- matrix(rep(0, (classes %>% levels %>% length) * 5), 
                       ncol = 5)
    Npost <- rep(list(NULL), classes %>% levels %>% length)
...
mult.synth.fit <- rep(list(NULL), 3)
mult.synth.fit[[1]] <- synth.fit()
mult.synth.fit[[2]] <- synth.fit(breaks = 
                                     c(210, 240, 270, 300, 330, 360, 400))
mult.synth.fit[[3]] <- synth.fit(breaks = c(400, 600, 800, 1600))
```

Analysis on the Behaviours of the Time-between-Events

```{r}
ECDF.interarrival <- function(i){
    
    samp.id <- mult.synth.fit[[i]]$environ$samp.id
    Nevents <- mult.synth$environ$Nevents
...
ecdf_interarrival <- lapply(seq_len(3), \(i) ECDF.interarrival(i))
```
